...
export const FormSchemaFranchiseForm = z.object({
  firstname: z.string().min(2).max(50),
  lastname: z.string().min(2).max(50),
  mobile: z.string().min(2).max(50),
  email: z.string().min(2).max(50),
...
export type TFormSchemaFranchiseForm = z.infer<typeof FormSchemaFranchiseForm>;

export const FormSchemaPendingInstructor = z.object({
...
export type TFormSchemaPendingInstructor = z.infer<
  typeof FormSchemaPendingInstructor
>;
...
export const FormSchemaAddFranchise = z.object({
  name: z.string().max(100).nullish(), // Optional name field
  profession: z.string().max(100).nullish(), // Optional profession field
  franchise_location: z.string().max(200).nullish(), // Optional franchise location
  budget: z.number().positive().nullish(), // Optional positive budget
  motivation: z.string().max(500).nullish(), // Optional motivation field
  expectations: z.string().max(500).nullish(), // Optional expectations field
  connection_type: z.string().max(50).nullish(), // Optional connection type
  requirement_stage: z.string().max(50).nullish(), // Optional requirement stage
  comments: z.string().max(500).nullish(), // Optional comments field
  email: z.string().email().max(100).nullish(), // Optional email field
  phone_number: z.string().max(15).nullish(), // Optional phone number
  city: z.string().max(100).nullish(), // Optional city field
  country: z.string().max(100).nullish(), // Optional country field
});

export type TFormSchemaAddFranchise = z.infer<typeof FormSchemaAddFranchise>;

export const FormSchemaSchoolForm = z.object({
...
export type TFormSchemaSchoolForm = z.infer<typeof FormSchemaSchoolForm>;
...
export const FormSchemaAddEmployee = z.object({
  employee_nid: z.string().max(50), // Required employee NID
  employee_name: z.string().max(100), // Required employee name
  employee_email: z.string().email().max(100), // Required employee email
  employee_gender: z.string().max(10), // Required gender
  employee_dateofbirth: z.date(), // Required date of birth
  employee_address: z.string().max(200), // Required home address
  employee_mobile: z.string().max(15), // Required mobile number
  employee_whatsapp: z.string().max(15), // Adjust for WhatsApp number format
  employee_dateofjoin: z.date(), // Required date of joining
  employee_roleid: z.string().max(50), // Required role ID
  employee_password: z.string().min(8), // Minimum length for passwords
  // Optional below
  ...
  employee_img: z
    ...
    .nullish(),
  employee_fathername_husbandname: z.string().max(100).nullish(), // Optional father/husband name
  employee_experience: z
    ...
    .nullish(), // Adjust as needed
  employee_religion: z.string().max(50).nullish(), // Optional religion
  employee_blood_group: z.string().max(10).nullish(), // Optional blood group
  employee_education: z.string().max(100).nullish(), // Optional education
  employee_language: z.string().max(50).nullish().default("English"),
  employee_timezone: z.string().max(50).nullish().default("UTC"),
  employee_username: z.string().max(50),
  employee_currency: z.string().max(50).nullish().default("USD"),
  employee_theme: z.string().max(50).nullish().default("light"),
});

export const BasePropertiesAdmin = z.object({
  admin_position: z.string().max(50).nullish().default("Manager"),
});
...
export const BasePropertiesInstructor = z.object({
  instructor_faculty: z.string().max(100),
  instructor_cv: z
    .any()
    .refine((file: File) => file?.size !== 0, "File is required")
    .refine(
  ...
  instructor_major: z.string().max(100),
});
